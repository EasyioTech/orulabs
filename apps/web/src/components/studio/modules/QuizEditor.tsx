...
import type { TrainingModule, ModuleConfig, QuizQuestion } from "@oruclass/types";
...
export function QuizEditor({
  ...
  const setQuestions = (next: QuizQuestion[]) => onChange({ ...config, questions: next });

  // Patch a single question by index; the one place question mutation happens.
  ...
  const inputCls =
    "px-3 py-2 bg-[#f1f3f4] border-b border-[#80868b] focus:border-b-2 focus:border-[#1a73e8] hover:bg-[#e8eaed] rounded-t-md outline-none transition-colors";
                    ...
                    <button
                      onClick={() =>
                        updateQuestion(qi, {
                          ...
                          correctAnswer: q.correctAnswer === opt ? undefined : q.correctAnswer,
                        })
                      }
            ...
            <div className="flex items-center gap-3 pl-1">
              <div className="flex items-center gap-2">
                ...
                <input
                  type="number"
                  value={q.minVal ?? 1}
                  onChange={(e) => updateQuestion(qi, { minVal: Number(e.target.value) })}
              ...
              <div className="flex items-center gap-2">
                ...
                <input
                  type="number"
                  value={q.maxVal ?? 10}
                  onChange={(e) => updateQuestion(qi, { maxVal: Number(e.target.value) })}
                  className={`w-16 text-xs ${inputCls}`}
                />
              </div>
            </div>
